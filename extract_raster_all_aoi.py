...
def get_aoi_id_from_label(label: str) -> int | None:
    """Parse the AOI id from a chunk label, e.g. 'AOI_5_ratio_0.05' -> 5."""
    parts = label.split("_")
    try:
        return int(parts[1])
...
def make_nan_row(aoi_id, lat, lon, vi_names, canopy_cover):
    row_nan = {"aoi_id": aoi_id, "latitude": lat, "longitude": lon, "canopy_cover": canopy_cover,
...
def export_and_process_aoi_chunk(chunk, aoi_id, polygon, lat, lon, vi_names, hsv_v_threshold, rdvi_threshold,
                                  vi_folder, vi_masked_folder, ortho_folder):
    """Export the clipped 5-band orthomosaic for one AOI chunk, compute VI stats, and write VI GeoTIFFs."""
...
if __name__ == "__main__":
    ...
    for exp in exps:
        ...
        for chunk in doc.chunks:
            aoi_id = get_aoi_id_from_label(chunk.label)
            if aoi_id is None:
            ...
            if chunk.orthomosaic is None:
                ...
                row_nan = make_nan_row(aoi_id, lat, lon, vi_names, np.nan)
                records_vi_masked.append(row_nan)
                records_vi_raw.append(row_nan)
                continue

            row, row_raw = export_and_process_aoi_chunk(
