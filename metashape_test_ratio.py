...
if __name__ == "__main__":
    ...
    for ratio in np.arange(0.05, 0.51, 0.05):
        ...
        try:
            doc.open(os.path.join(result_folder, f"{exp}_ratio_test.psx"))
        except:
            doc.save(os.path.join(result_folder, f"{exp}_ratio_test.psx"))
    ...
    results_df.to_csv(os.path.join(result_folder, f"processing_results_{aoi_id}.csv"), index=False)
