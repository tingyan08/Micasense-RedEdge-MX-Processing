...
def export_and_process_aoi(chunk, aoi_id, polygon, lat, lon, vi_names, hsv_v_threshold, rdvi_threshold,
                            vi_folder, vi_masked_folder, ortho_folder):
    """Export the clipped 5-band orthomosaic for one AOI square, compute VI stats, and write VI GeoTIFFs."""
    label = f"AOI_{aoi_id}"
    ...
    print("=" * 60)
    print(f"Processing AOI {aoi_id} at lat={lat:.6f}, lon={lon:.6f} with polygon bounds: {polygon.bounds}")

    # --- Export 5-band orthomosaic clipped to AOI ---
    ortho_path = os.path.join(ortho_folder, f"{label}.tif")
    chunk.exportRaster(
        path=ortho_path,
    ...
    if ortho_masked.shape[0] >= 6:
        nodata_mask = ortho_masked[5] == 0
    else:
    ...
    print(f"  Vegetation pixels after soil removal: {veg_count} / {total_valid}")

    row = {"aoi_id": aoi_id, "latitude": lat, "longitude": lon,
           "canopy_cover": veg_count / total_valid if total_valid > 0 else np.nan,
           "total_pixels": total_valid, "vegetation_pixels": veg_count}
    ...
    for vi_name in vi_names:
        vi_path        = os.path.join(vi_folder,        f"{label}_{vi_name}.tif")
        vi_masked_path = os.path.join(vi_masked_folder, f"{label}_{vi_name}_masked.tif")
...
if __name__ == "__main__":
    ...
    for root_folder in sorted(glob.glob(os.path.join(parent_folder, f"*_{field}"))):
        ...
        # Open Metashape project (single whole-field chunk)
        psx_path = os.path.join(result_folder, f"{exp}_whole_field.psx")
        ...
        try:
            ...
            raise FileNotFoundError(
                ...
                "Please run metashape_whole_field.py first."
            )
